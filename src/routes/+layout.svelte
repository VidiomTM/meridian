<script lang="ts">
	...
	let theme = $state(
		typeof localStorage !== 'undefined' ? (localStorage.getItem('meridian-theme') ?? 'paper') : 'paper'
	);
	let density = $state(
		typeof localStorage !== 'undefined' ? (localStorage.getItem('meridian-density') ?? 'comfortable') : 'comfortable'
	);
	let bodyType = $state(
		typeof localStorage !== 'undefined' ? (localStorage.getItem('meridian-bodytype') ?? 'serif') : 'serif'
	);
	...
	const activeDoc = $derived(activeId ? (data.byId[activeId] ?? data.byId[`${page.params.project}:${activeId}`] ?? null) : null);
	...
	$effect(() => {
		if (showGraph && !GraphViewComponent) {
			import('$lib/components/GraphView.svelte').then(m => (GraphViewComponent = m.default));
		}
		if (showHistory && !HistoryComponent) {
			import('$lib/components/History.svelte').then(m => (HistoryComponent = m.default));
		}
		if (showTweaks && !TweaksPanelComponent) {
			import('$lib/components/TweaksPanel.svelte').then(m => (TweaksPanelComponent = m.default));
		}
	});
	...
	onMount(() => {
		const onOpenGraph = () => { showGraph = true; };
		const onOpenHistory = () => { showHistory = true; };
	...
	function handleKeydown(e: KeyboardEvent) {
		...
		if (e.key === 'j' || e.key === 'k') {
			const proj = page.params.project;
			const fallback = proj
				? data.docs.filter((d: { project: string }) => d.project === proj).map((d: { id: string }) => d.id)
				: data.docs.map((d: { id: string }) => d.id);
			const list = explorerFilter.filteredIds.length > 0 ? explorerFilter.filteredIds : fallback;
